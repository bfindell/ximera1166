...
\begin{freeResponse}
\begin{hint}
The area of the rectangle is base times height.  The rectangle is made up of two congruent right triangles.  Because congruent triangles have the same area, the area of each right triangle is half the area of the rectangle.  
\end{hint}
\end{freeResponse}
\end{problem}
...
\[
\mathrm{area} =  \frac{hb_1}{2}+ \frac{hb_2}{2}= \frac{h(b_1 + b_2)}{2} = \frac{hb}{2}
\]
\end{hint}
because $b=b_1+b_2$.
\end{freeResponse}
...
\end{image}
One way to compute the area of the solid triangle is to compute the area right triangle that is the lower half of the rectangle (with base $b+x$) and subtract the area of the small right triangle (with base $x$): 
\[
\mathrm{area} =  \frac{h(b+x)}{2} - \frac{hx}{2}= \frac{hb}{2} + \frac{hx}{2} - \frac{hx}{2} = \frac{hb}{2}
\]
\end{hint}
\end{freeResponse}
\end{problem}
