...
\title{Interior Angles Measurement}
\author{Brad Findell}
\begin{document}
\begin{abstract}
...
\begin{problem}
Use the measurements from the previous problem, to computer the measure of the marked angle below.  
\begin{image}
...
\end{tikzpicture}
\end{image}
The marked angle should measure $\answer[tolerance=1.3]{79}$ degrees.  

% Compare to the measure of A B and C
\end{problem}

\begin{problem}
In order to explain why the sum of the interior angles should be $360^\circ$, Bart and Brad each \emph{triangulated} the figure as shown below.  
...
%\draw [fill=uuuuuu] (3.95,8.) circle (1.0pt);
%\draw[color=uuuuuu] (3.90,7.8) node {$E$};
\draw (3.9,7.5) node[align=center] {Brad's figure};
\end{scriptsize}
...
%\draw [fill=uuuuuu] (3.95,8.) circle (1.0pt);
%\draw[color=uuuuuu] (3.90,7.8) node {$E$};
\draw (3.9,7.5) node[align=center] {Bart's figure};
\end{scriptsize}
\end{scope}
\end{tikzpicture}
\end{image}
...
\begin{freeResponse}
\begin{hint}
In Bart's triangulation, the interior angles of the quadrilateral are composed only of interior angles of the triangular pieces.  But in Brad's figure, a new angle has been created between $A$ and $D$, and part of interior angle $C$ has been lost.  
\end{hint}
\end{freeResponse}
\end{problem}
