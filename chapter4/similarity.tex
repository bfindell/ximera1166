...
\begin{question}
Formally, two figures are \textbf{congruent} if there is a sequence of basic $\answer[format=string]{rigid motions}$ (two words) that maps one onto the other.  
\end{question}
\end{question}

\begin{question}
%Describe, both informally and formally, what it means to say two figures are similar.
Informally, figures that have the same shape are said to be $\answer[format=string]{similar}$.  With moving and scaling (zooming in or out), one can be placed on another so that the figures match exactly.    
\begin{question}
...
\end{enumerate}
  ...
  Explain. 
\begin{question}
Scale any Pythagorean triple by an integer scale factor greater than 1, and you have another Pythagorean triple.  For example, because \{3, 4, 5\} is a Pythagorean triple, so is \{$\answer{3n}$, $\answer{4n}$, $\answer{5n}$\}, where $n \ge 1$ is an $\answer[format=string]{integer}$.  
\end{question}
\end{question}
...
\begin{question}
Suppose you have two similar triangles with scale factor $r$. Is it possible to state relationships between their perimeters?  Their areas?
