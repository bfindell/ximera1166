...
\end{itemize}
I have read the above notes and will consider the following proofs and 
non-proofs $\answer[format=string]{separately}$.  
\end{problem}
...
Taylor asks, ``How do we know the perpendicular bisector will go through point $C$?  Maybe the figure looks like this!'' She presents the third figure. 

Without using other facts about isosceles triangles or perpendicular bisectors, choose the best judgment of their disagreement: 
...
If it were valid to ignore the given (i.e., that the triangle is isosceles), then the base angles of \textbf{any} triangle would be congruent.
\end{feedback}
...
\end{multipleChoice}
\begin{feedback}[correct]
Ignoring the marking that $\overline{CD}\perp \overline{AB}$ would mean that $\overline{CD}$ merely a median, and the proof could be fixed.  
\end{feedback}  
\end{problem}
\end{problem}
\end{problem}
