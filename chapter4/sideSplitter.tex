...
Three ways of thinking about triangle similarity:  
\begin{itemize}
\item Sequences of basic rigid motions and dilations. 
\item Ratios of corresponding side lengths within or between figures. 
\item AAA criterion.  
\end{itemize}
...
\begin{question}
State the \textbf{Parallel-Side Theorem}:  If a line in a triangle is $\answer[format=string]{parallel}$ to a side of a triangle, then it splits the other sides of the triangle $\answer[format=string]{proportionally}$. 
\begin{image}
\includegraphics[scale=0.8]{sideSplitter5}
\end{image}
That is, $\frac{DB}{AD} = \frac{\answer{EC}}{AE}$.  

Is it also true that $\frac{DB}{AD} = \frac{BC}{DE}$?  $\answer[format=string]{no}$
\begin{question}
Correct:  Because 
\end{question}
\end{question}
...
\begin{enumerate}
\item A dilation takes a line not passing through the center of the dilation to a parallel line
\item A dilation leaves a line passing through the center unchanged.
