...
\begin{enumerate}
\item $\triangle ABC\sim \triangle \answer[format=string]{DBA}$ by AA similarity because they share $\angle B$ and they each have a right angle. 

\item $\triangle ABC\sim \triangle \answer[format=string]{DAC}$ by AA similarity because they share $\angle C$ and they each have a right angle. 

\item $\triangle DAC\sim \triangle \answer[format=string]{DBA}$ because they are both similar to
$\triangle ABC$.
\end{enumerate}
\fixnote{Should we prompt for AA in the first two steps? The 2017 EOC item prompts for ``AA postulate,'' but AA is not a postulate.  \\Should AA be called a criterion?  a theorem? a condition? a test? Is ``AA similarity'' enough?}
\end{problem}
