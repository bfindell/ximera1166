...
\draw [color=white] (-4.,0.) circle (0.1pt);
\draw [color=white] (8.,0.) circle (0.1pt);
\draw[color=black] (1.6,-0.2) node {$b_1$};
\draw[color=black] (3.5,-0.2) node {$b_2$};
\draw[color=black] (2.8,1.2) node {$h$};
\end{tikzpicture}
%\includegraphics{pbpDisTri.png}
\end{image}

Then $b=b_1+b_2$, and the three triangles all have the same height, $h$.   The area of the whole triangle is the sum of the areas of the two right triangles:  
\[
...
\draw [line width=0.8pt] (0.,0.)-- (2.,0.);
\draw [line width=0.8pt,dash pattern=on 2pt off 2pt] (2.,0.)-- (5.,0.);
\draw[color=black] (1.1,-0.2) node {$4$};
\draw[color=black] (3.5,-0.2) node {$6$};
\draw[color=black] (5.2,1.3) node {$5$};
%\draw[step=0.5,black,thin,xshift=0.5,yshift=0.5] (0.0,0.0) grid (5.0,2.5);
\draw [color=white] (-4.,0.) circle (0.2pt);
\draw [color=white] (8.,0.) circle (0.2pt);
\end{tikzpicture}
%\includegraphics{triangleArea.png}
\end{image}
Find the area of this triangle (which is much less than half the rectangle).  Then generalize the approach. 
\end{hint}
