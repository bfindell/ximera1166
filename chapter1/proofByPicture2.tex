...
\begin{problem}
Explanation: Draw a $\answer[format=string]{diagonal}$ of the parallelogram and the heights.  
\begin{image}
%\includegraphics{para.png}
\begin{tikzpicture}[scale=0.8,line cap=round,line join=round,>=triangle 45,x=1.0cm,y=1.0cm]
\draw [line width=0.8pt] (0.,0.) -- (2.,3.) -- (8.,3.) -- (6.,0.) -- cycle;
...
\draw [line width=0.8pt,dash pattern=on 3pt off 3pt] (0.,0.) -- (0.,3.) -- (2,3);
\draw [line width=0.8pt,dash pattern=on 3pt off 3pt] (6.,0.) -- (8.,0.) -- (8,3);
\draw (3.3,-0.3) node {$b$};
\draw (4.7,3.3) node {$b$};
\draw (8.3,1.5) node {$h$};
\draw (-0.3,1.5) node {$h$};
...
\end{tikzpicture}
\end{image}
Then there are two congruent $\answer[format=string]{triangles}$ with the same $\answer[format=string]{base}$ and 
the same $\answer[format=string]{height}$ as the parallelogram.  
\[
\mathrm{Area} =  \answer{\frac{hb}{2}} + \answer{\frac{hb}{2}}= hb,
\]
which is the base times the height. 
\end{problem}
\end{problem}
...
Help Giorgio out by \textbf{selecting all} correct statements:   

\begin{selectAll}
\choice{Jenny's cutting and rearranging argument works for this parallelogram.}  
\choice[correct]{A different cutting and rearranging argument is needed for this parallelogram.} 
\choice[correct]{Yet a different parallelogram might require yet another cutting and rearranging approach.}
\choice{All this discussion is a waste of time:  Just use the formula!}
\end{selectAll}
...
\begin{problem}
This parallelogram problem is much like Giorgio's triangle problem.  Here is Giorgio's final explanation: (1) compute the area of a larger figure (a rectangle with base $\answer{b+x}$ and height $\answer{h}$) and then (2) $\answer[format=string]{subtract}$ the areas we don't want (this time two congruent right triangles with base $\answer{x}$ and height $\answer{h}$): 
\begin{image}
\begin{tikzpicture}[scale=0.8,line cap=round,line join=round,>=triangle 45,x=1.0cm,y=1.0cm]
%\includegraphics{paragiorgio.png}
\draw [line width=0.8pt] (0.,0.)-- (5.,3.) -- (8.,3.)-- (3.,0.) -- cycle;
...
\begin{align*}
\mathrm{Area} &=  \answer{h(b+x)} - \answer{\frac{hx}{2}} - \answer{\frac{hx}{2}} \\
 &= hb + hx - \frac{hx}{2}- \frac{hx}{2} \\
 &= \answer{bh}.
\end{align*}

Given a base and a height of a parallelogram, \textbf{select all} true statements:
\begin{selectAll}
\choice{Jenny's explanation applies to all parallelograms.}
\choice[correct]{Jenny's explanation is a useful intuitive way of thinking about parallelogram area, though it is not completely general.}
\choice[correct]{Giorgio's final explanation applies to all parallelograms.}
\choice{Giorgio's explanation is a useful intuitive way of thinking about parallelogram area, though it is not completely general.}
\choice{Each parallelogram is different, so we cannot say.}
\end{selectAll}
