...
\draw[color=qqqqff] (0.2,2.29) node {$B$};
\draw [fill=qqqqff] (0.,-5.) circle (1.2pt);
\draw[color=qqqqff] (-0.28,-5.15) node {$D$};
%\draw [fill=qqqqff] (0.,0) circle (1.2pt);
...
The figure below is sometimes called a \emph{dart} (or \emph{arrowhead}).  Is it a kite?  

\begin{multipleChoice}
\choice{Yes.}
\choice{No.}
\choice[correct]{It depends.}
\end{multipleChoice}
...
\begin{problem}
Correct!  Some definitions require a kite to be a \emph{convex} polygon, which is to say that it contains every segment connecting two points on the polygon.  A dart can be seen as \emph{concave} (i.e., non-convex) kite from the fact that diagonal $\overline{AC}$ (for example) is not contained in the kite.  

What about the theorem that a diagonal of a kite is the perpendicular bisector of the other diagonal?  Does it work for a dart?  

\begin{multipleChoice}
\choice{Yes.}
\choice{No.}
\choice[correct]{Yes, with some modification.}
\end{multipleChoice}
...
\begin{selectAll}
\choice[correct]{The first proof applies to a dart.}
\choice{The first proof does not apply to a dart.}
\choice{The first proof would require some modification.}
\choice[correct]{The second proof applies to a dart.}
\choice{The second proof does not apply to a dart.}
\choice{The second proof would require some modification.}
\choice{Both proofs need to be completely rewritten.}
\end{selectAll}
\begin{feedback}
